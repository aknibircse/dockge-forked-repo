...
async function initRandomBytes() {
    if (typeof window !== "undefined" && window.crypto) {
        randomBytes = function randomBytes(numBytes) {
...
export function statusNameShort(status) {
    switch (status) {
        case CREATED_FILE:
            return "inactive";
        case CREATED_STACK:
...
export const allowedCommandList = [
    "docker",
    "ls",
    "cd",
    "dir",
 ...
 */
export function intHash(str, length = 10) {
...
export function getComposeTerminalName(endpoint, stack) {
    return "compose-" + endpoint + "-" + stack;
}
export function getCombinedTerminalName(endpoint, stack) {
    return "combined-" + endpoint + "-" + stack;
}
export function getContainerTerminalName(endpoint, container) {
    return "container-" + endpoint + "-" + container;
}
export function getContainerExecTerminalName(endpoint, stackName, container, index) {
    return "container-exec-" + endpoint + "-" + stackName + "-" + container + "-" + index;
}
export function copyYAMLComments(doc, src) {
...
function copyYAMLCommentsItems(items, srcItems) {
    if (!items || !srcItems) {
        return;
    }
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const srcIndex = srcItems.findIndex((srcItem) => JSON.stringify(srcItem.value) === JSON.stringify(item.value) &&
 ...
 * @param hostname
 */
export function parseDockerPort(input, hostname) {
    let port;
    let display;
...
export function envsubst(string, variables) {
 ...
 */
export function envsubstYAML(content, env) {
 ...
 * @param env
 */
function traverseYAML(pair, env) {
    // @ts-ignore
