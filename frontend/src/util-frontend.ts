...
export function timezoneList() {
    // Define interface for timezone result items
 ...
 */
export function getToastSuccessTimeout(): number | boolean {
    let successTimeout: number | boolean = 20000;
    ...
    if (successTimeout === -1) {
        // Use type assertion to tell TypeScript we're intentionally changing the type
        successTimeout = false as any;
    }
 ...
 * @returns {(number|boolean)} Timeout in ms. If false timeout disabled.
 */
export function getToastErrorTimeout(): number | boolean {
    let errorTimeout: number | boolean = -1;
