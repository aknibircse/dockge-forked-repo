...
export class TerminalSocketHandler extends AgentSocketHandler {
    create(socket, server, agentSocket) {
        agentSocket.on("terminalInput", async (terminalName, cmd, callback) => {
            try {
                checkLogin(socket);
        ...
        // Main Terminal
        agentSocket.on("mainTerminal", async (terminalName, callback) => {
            try {
                checkLogin(socket);
        ...
        agentSocket.on("interactiveTerminal", async (stackName, serviceName, shell, callback) => {
            try {
                checkLogin(socket);
        ...
        agentSocket.on("terminalJoin", async (terminalName, callback) => {
            ...
            try {
                ...
                let buffer = Terminal.getTerminal(terminalName)?.getBuffer() ?? "";
                if (!buffer) {
        ...
        agentSocket.on("leaveCombinedTerminal", async (stackName, callback) => {
            try {
                checkLogin(socket);
        ...
        // Resize Terminal
        agentSocket.on("terminalResize", async (terminalName, rows, cols) => {
